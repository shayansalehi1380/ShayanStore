...
namespace AdminPanel.UI.Controllers
{
    ...
    {
        public async Task<ActionResult<List<FeatureDetails>>> GetAllFeatureDetails(string? searchFeatureDetails, int tabs = 2)
        {
            ...
            if (!string.IsNullOrEmpty(searchFeatureDetails))
            {
                queryFeatureDetails = queryFeatureDetails.Where(x => x.Title.Contains(searchFeatureDetails) || x.Feature.Title.Contains(searchFeatureDetails));
            }
        ...
        {
            var featureDetails = await unitOfWork.GenericRepository<FeatureDetails>().GetByIdAsync(id, CancellationToken.None);
